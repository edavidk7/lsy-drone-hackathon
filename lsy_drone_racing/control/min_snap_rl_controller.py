"""Collision-aware min-snap path tracked by the trained PPO policy, with re-planning.

This combines two pieces:

* **Planning** -- :class:`~lsy_drone_racing.control.min_snap_controller.MinSnapController` builds a
  smooth, dynamically feasible, collision-aware trajectory through the gates (perpendicular gate
  crossings, pole + gate-frame keep-outs, snap minimization, acceleration-limited time scaling).
  See ``min_snap_controller.py``.
* **Tracking** -- the trained line-tracking PPO policy (``ppo_drone_racing.ckpt``) consumes a *path*
  as a set of look-ahead points and outputs collective-thrust + attitude commands. It learned the
  drone dynamics, so it tracks tight turns far more gracefully than the built-in onboard state
  controller.
...
class MinSnapRLController(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        # Checkpoint selection. ``[controller] model_path`` in the track toml overrides everything
        # (lets you A/B different checkpoints without code edits; relative paths resolve under
        # ``control/``). Otherwise prefer the policy retrained on our min-snap gate courses
        # (``train_min_snap_rl.py`` -> ``ppo_min_snap.ckpt``), falling back to the stock checkpoint.
        cfg_model = getattr(config.controller, "model_path", None)
        if cfg_model:
            model_path = Path(cfg_model)
