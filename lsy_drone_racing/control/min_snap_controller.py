"""Collision-aware min-snap path planner (geometric, gate-relative) for arbitrary tracks.

This module is the standalone *planner* extracted from the self-contained min-snap controller. It
plans **once** at construction time from the gate poses in ``obs`` (never hardcoded):
...
2. Insert free *via* points between consecutive fixed points.
...
The planner exposes the optimized quintic spline (position + velocity + acceleration via analytic
derivatives) through :attr:`MinSnapController.spline` / :attr:`MinSnapController.t_total`, plus a
:meth:`MinSnapController.replan` fast path for re-planning inside the control loop. It is consumed
by :mod:`lsy_drone_racing.control.min_snap_rl_controller`, where the trained line-tracking PPO
policy tracks the planned path. It can also still be used standalone as a ``Controller`` that emits
a state setpoint (pos/vel/acc feedforward).
...
.. note::
    ...
    that, but the full optimization takes ~1 s so it should not run inside the 50 Hz control loop
    (use ``replan(optimize=False)`` for the in-loop fast path).
"""
